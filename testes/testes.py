...
def gerar_recomendacao_completa(
    ...
    # Ajuste fino para instrumentalness e speechiness
    # Se a persona prefere 'speechiness' alta, reduz o peso de 'instrumentalness' e vice-versa
    ...
    if not playlist_cluster.empty:
        print("\n=== MÚSICAS SELECIONADAS DOS CLUSTERS RELEVANTES ===")
        print_playlist_bonita(playlist_cluster.head(num_recommendations_needed))
    else:
        print("\n=== NENHUMA MÚSICA SELECIONADA DOS CLUSTERS RELEVANTES. USANDO A LÓGICA DE FALLBACK ===")

    playlist_final = playlist_cluster.head(num_recommendations_needed).copy()

    # 4. Fallback final se a playlist ainda não estiver cheia (AGORA MAIS INTELIGENTE)
    if len(playlist_final) < num_recommendations_needed:
        ...
        # === NOVOS PASSOS DO FALLBACK INTELIGENTE ===
        ...
        if faltantes > 0:
            ...
            if len(musicas_genero_match) > faltantes:
                playlist_final = pd.concat([playlist_final, musicas_genero_match.head(faltantes)], ignore_index=True)
                print(
                    f"INFO: Fallback 2 ativado: {len(musicas_genero_match.head(faltantes))} músicas com gênero correspondente adicionadas.")
                faltantes = num_recommendations_needed - len(playlist_final)
        ...
        if faltantes > 0:
            ...
            print(f"INFO: Fallback 3 ativado: {faltantes} músicas genéricas adicionadas.")
