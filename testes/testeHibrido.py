...
def filtrar_playlist(df_musicas, generos_mapeamento, persona, pesos, top_n=20, generos_indesejados=None):
    ...
    for _, row in df.iterrows():
        ...
        if not any(g in generos_persona_mapeados for g in musica_generos):
            df.loc[row.name, 'final_score'] *= 0.5
...
# --- NOVAS FUNÇÕES ---
def avaliar_e_ajustar_pesos(pesos_atuais, notas_playlist, feedback_text):
    """
    Simula uma avaliação da persona e ajusta os pesos do algoritmo se a nota média for baixa.
    """
    try:
        nota_media = np.mean(notas_playlist)
        print(f"\n--- Avaliação da Persona ---")
        print(f"Nota média da playlist: {nota_media:.2f}/10")
        print(f"Feedback da Persona: '{feedback_text}'")

        if nota_media < 7.0:
            print("NOTA BAIXA: Reajustando pesos para a próxima recomendação...")
            pesos_ajustados = pesos_atuais.copy()
            # Aumenta o peso do gênero, pois a nota baixa pode indicar
            # que a playlist não correspondeu bem ao estilo principal da persona.
            pesos_ajustados['genero'] *= 1.15
            # Aumenta o peso da popularidade para garantir que músicas mais conhecidas entrem na playlist.
            pesos_ajustados['popularity'] *= 1.10
            # Reduz o peso de características menos importantes, como a 'liveness' ou 'speechiness'
            pesos_ajustados['liveness'] *= 0.95
            pesos_ajustados['speechiness'] *= 0.95

            soma_pesos = sum(pesos_ajustados.values())
            pesos_normalizados = {k: v / soma_pesos for k, v in pesos_ajustados.items()}
            ...
            print("NOTA ALTA: Pesos atuais mantidos.")
            return pesos_atuais

    except Exception as e:
        print(f"Erro ao avaliar e ajustar pesos: {e}. Pesos atuais mantidos.")
        return pesos_atuais
...
def gerar_recomendacao_completa(
    ...
    if pesos_atuais:
        ...
        # Ajusta pesos com base nas preferências da persona
        for feature in ['energy', 'tempo', 'liveness']:
            pref = novo_usuario.get(feature, 'medio')
            if pref == 'alto':
                pesos_base[feature] += 0.05
            elif pref == 'baixo':
                pesos_base[feature] += 0.02
    ...
    if not playlist_cluster.empty:
        ...
        playlist_final = selecionar_musicas_diversas(
            ...
            artist_cap=1,  # no máximo 1 música por artista
            genre_cap=3,  # no máximo 3 músicas por gênero
...
if musicas_base is not None:
    ...
    for i in range(3):
        ...
        if reacao:
            ...
            # Simula a avaliação do usuário/persona (notas de 1 a 10)
            notas_feedback = np.random.randint(5, 11, size=len(playlist))
            # O feedback de texto pode ser extraído de uma análise do próprio Gemini
            feedback_text = "As músicas têm o ritmo certo, mas poderiam ser mais parecidas com o meu artista favorito."

            # Recalcula os pesos com base no feedback
            pesos_atuais = avaliar_e_ajustar_pesos(pesos_dinamicos_usados, notas_feedback, feedback_text)
